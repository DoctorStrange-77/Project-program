      ...
      <div className="bg-[#121212] border border-white/5 rounded-2xl p-6 sm:p-8 space-y-4">
        <div>
          <h3 className="font-black text-white text-sm uppercase tracking-wider flex items-center gap-2">
            ...
            Backup dei Dati Locali
          </h3>
          <p className="text-[11px] text-white/40 font-medium">Esporta tutti i tuoi clienti, le schede di allenamento e le configurazioni in un file JSON per sicurezza, o importali su un altro dispositivo.</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 pt-2">
          <button
            ...
            onClick={() => {
              try {
                const backupData = {
                  config: localStorage.getItem('pt_coach_config'),
                  clients: localStorage.getItem('pt_clients'),
                  exercises: localStorage.getItem('pt_exercises'),
                  plans: localStorage.getItem('pt_plans'),
                  templates: localStorage.getItem('pt_templates')
                };
                ...
                if (onShowToast) {
                  onShowToast('Backup esportato con successo!', 'success');
                }
              } catch (err) {
                if (onShowToast) {
                  onShowToast('Errore durante l\'esportazione del backup.', 'error');
                }
              }
            }}
            className="flex items-center justify-center gap-2 px-5 py-3 rounded-xl bg-neutral-900 border border-white/5 hover:bg-neutral-800 text-white font-bold text-xs uppercase cursor-pointer transition-colors"
          >
            <Download className="w-4 h-4" style={{ color: primaryColor }} />
            Esporta Backup (JSON)
          </button>

          <label className="flex items-center justify-center gap-2 px-5 py-3 rounded-xl bg-neutral-900 border border-white/5 hover:bg-neutral-800 text-white font-bold text-xs uppercase cursor-pointer transition-colors">
            <Upload className="w-4 h-4 text-[#CCFF00]" style={{ color: primaryColor }} />
            Importa Backup (JSON)
            <input
              type="file"
              accept=".json"
              onChange={(e) => {
                ...
                reader.onload = (event) => {
                  try {
                    ...
                    if (data.config) localStorage.setItem('pt_coach_config', data.config);
                    if (data.clients) localStorage.setItem('pt_clients', data.clients);
                    if (data.exercises) localStorage.setItem('pt_exercises', data.exercises);
                    if (data.plans) localStorage.setItem('pt_plans', data.plans);
                    if (data.templates) localStorage.setItem('pt_templates', data.templates);
                    
                    if (onShowToast) {
                      onShowToast('Backup importato correttamente! Ricaricamento in corso...', 'success');
                    }
                    ...
                    if (onShowToast) {
                      onShowToast('Errore durante l\'importazione. File non valido.', 'error');
                    }
                  }
                };
